...
wasmAritateFunctii={"i32.add":2, "i32.sub":2, "i32.mul":2, "i32.div_s":2, "i32.div_u":2, "i32.rem_s":2, "i32.rem_u":2, "i32.and":2, "i32.or":2, "i32.xor":2, "i32.shl":2, "i32.shr_s":2, "i32.shr_u":2, "i32.rotl":2, "i32.rotr":2,\
				...
				"i64.extend32_s":1, "i64.extend_i32_u":1}
wasmFunctiiBaza={"i32.add":i32.i32.add, "i32.sub":i32.i32.sub, "i32.mul":i32.i32.mul, "i32.div_s":i32.i32.div_s, "i32.div_u":i32.i32.div_u, "i32.rem_s":i32.i32.rem_s, "i32.rem_u":i32.i32.rem_u, "i32.and":i32.i32._and, "i32.or":i32.i32._or, "i32.xor":i32.i32._xor, "i32.shl":i32.i32.shl, "i32.shr_s":i32.i32.shr_s, "i32.shr_u":i32.i32.shr_u, "i32.rotl":i32.i32.rotl, "i32.rotr":i32.i32.rotr,\
			 ...
			 "i64.extend32_s":i64.i64.extend32_s, "i64.extend_i32_u":i64.i64.extend_i32_u}
...
class Interpretor:
	...
	def wasmCallFunc(self, ast, F):
		...
		while len(locVar)<len(F.paramTypes):
			...
			if not isinstance(x, retType):
				return f"type mismatch in function {F.callName if F.callName!='' else F.invokeName} parameter load"
			locVar.append(x)
		...
		if isinstance(F.AST, AST.AST):
			...
			if error!="":
				if isinstance(error, tuple) and error[0]=="return from function":
					...
					return f"{error} in function {F.callName if F.callName!='' else F.invokeName}"
			if len(F.results)!=(l:=len(x)):
				return f"function {F.callName if F.callName!='' else F.invokeName} is expected to return {len(F.results)} values but returns {l}"
			for i in range(l):
				if wasmDataTypes[F.results[i]]!=type(x[i]):
					return f"function {F.callName if F.callName!='' else F.invokeName} is expected to return {F.results[i]} but returns (type={type(x[i])}, value={x})"
			self.wasmStack[-1].extend(x)
			return ""
		
		if F.results!=[]:
			return f"function {F.callName if F.callName!='' else F.invokeName} is expected to return, but has no implementation"
		return ""
	...
	def wasmEvalIf(self, ast):
		...
		#(if [(param ...)] [(result ...)] [(conditie)] (then ...) [(else ...)])
		resultType=[]
		conditie=""
		thenInstr=""
		elseInstr=""
		...
		myName=""
		if part=="label":
			self.wasmBlockAlias[myName:=ast.children[self.wasmPozEval[-1]]]=self.wasmBlocksCount
			self.wasmPozEval[-1]+=1
			part=Interpretor.wasmEvalIfHelper(ast.children[self.wasmPozEval[-1]])
		self.wasmBlocksCount+=1
		if part=="param":
			listaParametri=[]
			cnt=len(ast.children[self.wasmPozEval[-1]].children)-1
			if cnt>len(self.wasmStack[-1]):
				return "type mismatch"
			listaParametri=self.wasmStack[-1][-cnt:]
			self.wasmStack[-1]=self.wasmStack[-1][:-cnt]
			for i in range(cnt):
				if not isinstance(listaParametri[i], wasmDataTypes[ast.children[self.wasmPozEval[-1]].children[i+1].token]):
					return "type mismatch"
			self.wasmPozEval[-1]+=1
			part=Interpretor.wasmEvalIfHelper(ast.children[self.wasmPozEval[-1]])
		if part=="result":
		...
		if x._val!=0:
			...
			if thenInstr!="":
				...
				self.wasmPozEval.pop()
				if error!="":
					if isinstance(error, tuple) and error[0]=="skip to block index" and error[1]==self.wasmBlocksCount-1:
						if myName!="":
							del self.wasmBlockAlias[myName]
						self.wasmStack.pop()
		...
		#verifica tipurile de date returnate
		ret_ans=self.wasmStack.pop()
		for i in range(len(resultType)):
			if not isinstance(ret_ans[i], resultType[i]):
				if myName!="":
					del self.wasmBlockAlias[myName]
				self.wasmBlocksCount-=1
				return "type mismatch"
		self.wasmStack[-1]+=ret_ans
	...
	def wasmEvalKeyword(self, ast):
		...
		if t.token=="invoke":
			...
			if ast.children[self.wasmPozEval[-1]].token not in self.functiiWasm:
				return "unexpected token "+str(ast.children[self.wasmPozEval[-1]].token)
		...
		if t.token=="return":
			...
			if self.wasmPozEval[-1]<len(ast.children):
				...
				self.wasmStack[-1]=self.wasmStack[-2]
		...
		if t.token=="br_if":
			...
			if ast.children[self.wasmPozEval[-1]].tokType=="alias":
				...
				brIndex=Interpretor.wasmTokenToNumber(ast.children[self.wasmPozEval[-1]])
				if brIndex>=self.wasmBlocksCount:
					return f"unknown label {brIndex} >= {self.wasmBlocksCount}"
			else:
	...
	def wasmEvalAssert(self, ast):
		...
		if ast.children[self.wasmPozEval[-1]].token == "assert_invalid":
			...
			if isinstance(err, tuple):
				print(err)
			if "expected" in err.lower() or "mismatch" in err.lower() or "operator" in err.lower() or "unknown" in err.lower():
				return "ok"
		...
		if ast.children[self.wasmPozEval[-1]].token=="assert_malformed": # verificam daca ceea ce e pus intre ghilimele in modulul cu quote e ok
			...
			T=tokenizer.Tokenizer(codNou)
			A=AST.makeAST(T.tokens)
			A.children.insert(2, AST.AST())
			numeFunctie=self.getFunctionNameFromModule(A)
			A.children[-1].children = [tokenizer.Token("keyword", "call"), tokenizer.Token("alias", numeFunctie)]
			errCode = ASTChecker.ASTChecker().checkAST(A)
			if errCode != "seems fine":
				codEroare=errCode
			else:
	...
	def wasmEvalIf(self, ast):
		...
		if part=="param":
			...
			if cnt>len(self.wasmStack[-1]):
				return "type mismatch"
			for i in range(cnt):
				if ast.children[self.wasmPozEval[-1]].children[i+1].token in wasmDataTypes:
					listaParametri.append(wasmDataTypes[ast.children[self.wasmPozEval[-1]].children[i+1].token])
				else:
					if myName!="":
						del self.wasmBlockAlias[myName]
		...
		if cnt:
			params=self.wasmStack[-1][-cnt:]
			self.wasmStack[-1]=self.wasmStack[-1][:-cnt]
			for i in range(cnt):
				if not isinstance(params[i], listaParametri[i]):
					return "type mismatch"
		else:
			params=[]
			
		self.wasmStack.append(params)
		self.wasmBlocksCount+=1
			...
			if thenInstr!="":
				...
				if error!="":
					if isinstance(error, tuple) and error[0]=="skip to block index" and error[1]==self.wasmBlocksCount-1:
						...
						print(error, self.wasmBlocksCount)
						if myName!="":
							del self.wasmBlockAlias[myName]
						self.wasmBlocksCount-=1
						return error
	...
	def wasmEvalBlock(self, ast: AST.AST):
		...
		for i in range(len(x)):
			if type(x[i])!=tipuriReturn[i]:
				...
				return f"block is meant to return {tipuriReturn[i]} as it's {i}-th return but returns (type={type(x)}, value={x})"
		self.wasmStack[-1].extend(x)
		if myName!="" and myName in self.wasmBlockAlias:
			del self.wasmBlockAlias[myName]
		self.wasmBlocksCount-=1
		return ""
